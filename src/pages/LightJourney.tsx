...
export default function LightJourney() {
  ...
  function start() {
    ...
    intervalRef.current = window.setInterval(() => {
      ...
      if (elapsedRef.current >= minutes * 60) {
        finish();
      }
    }, 1000);
  }
  ...
  function finish() {
    ...
    logSession({
      category: 'light',
      label: `Podróż światła — ${THEMES[theme].name}`,
  ...
  function stop() {
    ...
    if (elapsedRef.current > 8) {
      logSession({
        category: 'light',
        label: 'Podróż światła (zakończona wcześniej)',
        durationSec: elapsedRef.current,
        meta: { theme, cycleSec },
      });
    }
    ...
    return (
      <div className="flex flex-col items-center gap-5">
        <div
          ...
          style={{ background: t.base, height: 'min(70vh, 560px)' }}
        >
          <div
            className="light-pulse-overlay absolute inset-0"
            style={{
              background: t.glow,
              animation: `light-pulse ${cycleSec}s ease-in-out infinite`,
            }}
          />
  ...
  if (done) {
    return (
      <Card className="mx-auto max-w-md text-center">
        ...
        <p className="mt-2 text-lg font-semibold">Podróż zakończona</p>
        <p className="mt-1 text-[var(--color-muted)]">Usiądź chwilę w ciszy, zanim wrócisz do codziennych aktywności.</p>
  ...
  return (
    <div>
      <SectionTitle
        ...
        description="Łagodnie pulsujące światło w rytmie oddechu, inspirowane doświadczeniami typu light journey (np. Lumenate). Wersja spowolniona i bez migotania, dla bezpieczeństwa."
      />

      <Card className="mb-6 border-rose-400/25 bg-rose-400/5">
        <p className="text-sm text-rose-200">
          🚫 <strong>Nie używaj tej funkcji, jeśli masz padaczkę światłoczułą, migreny wywoływane światłem lub inne
          schorzenia neurologiczne wrażliwe na bodźce wizualne.</strong> To nie jest stroboskop — światło pulsuje bardzo
          wolno (pełny cykl trwa {cycleSec}s), ale jeśli poczujesz zawroty głowy, mdłości, dezorientację lub
          jakikolwiek dyskomfort — natychmiast zakończ sesję i otwórz oczy. Nie używaj podczas prowadzenia pojazdu.
        </p>
      </Card>
      ...
      {!acknowledged ? (
        <Card className="mx-auto max-w-xl text-center">
          <label className="flex items-start gap-3 text-left text-sm text-[var(--color-muted)]">
            ...
            />
            <span>
              Przeczytałem/-am ostrzeżenie powyżej i potwierdzam, że nie mam padaczki światłoczułej ani innych
              przeciwwskazań.
            </span>
          </label>
        </Card>
        ...
        <Card className="mx-auto max-w-xl">
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-[var(--color-muted)]">Dłuższy cykl = wolniejsze, łagodniejsze pulsowanie.</p>
            </div>
            <div>
              <div className="mb-1 flex justify-between text-sm text-[var(--color-muted)]">
